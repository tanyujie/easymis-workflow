...
@RestController
@Api(value = "菜单管理 controller", tags = { "菜单管理  操作接口" })
@RequestMapping("/organize/resource")
public class OrganizeResourceController {    
	@Autowired
	...
	public ModelAndView index1(HttpSession httpSession) {
		...
		ModelAndView mv = new ModelAndView("organize/resource/index");
		return mv;
	}
	...
	public RestfulMessage saveCategory(HttpSession httpSession, @RequestBody OrganizePosition bean) {
		try {
			httpSession.getAttribute("userLogin");
			service.save(bean);
			return new RestfulMessage().success("保存菜单成功");
		} catch (Exception e) {
			return new RestfulMessage().success("保存菜单失败");
		}

	}
	//修改
	@RequestMapping(value = "/update/{id}", method = RequestMethod.PUT)  
	public RestfulMessage  updateCategory(HttpSession httpSession,@PathVariable("id") String id,@RequestBody OrganizePosition bean) {
		try {
			httpSession.getAttribute("userLogin");
			service.update(bean);
			return new RestfulMessage().success("更新菜单成功");
		} catch (Exception e) {
			return new RestfulMessage().success("更新菜单失败");
		}
	...
	public RestfulMessage  getCategory(HttpSession httpSession,@PathVariable("id") String id) {
		try {
			...
			return new RestfulMessage().success("获取菜单失败");
		}
	...
	public RestfulMessage delete(HttpSession httpSession, @PathVariable("id") String id) {
		try {
			httpSession.getAttribute("userLogin");
			service.delete(id);
			return new RestfulMessage().success("删除菜单成功");
		} catch (Exception e) {
			return new RestfulMessage().success("删除菜单失败");
		}
