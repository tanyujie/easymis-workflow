...
@RestController
@Api(value = "表单定义controller", tags = { "表单操作接口" })
@RequestMapping("/form/define")
public class FormDefineControlle {
    @Autowired
	...
	public RestfulMessage saveCategory(HttpSession httpSession, @RequestBody FormDefine bean) {
		try {
			httpSession.getAttribute("userLogin");
			service.save(bean);
	...
	public RestfulMessage  updateCategory(HttpSession httpSession,@PathVariable("id") String id,@RequestBody FormDefine bean) {
		try {
			httpSession.getAttribute("userLogin");
			service.update(bean);
	...
	public RestfulMessage  getCategory(HttpSession httpSession,@PathVariable("id") String id) {
		try {
			httpSession.getAttribute("userLogin");
			FormDefine bean=service.get(id);
	...
	public DataTableResult listJson(HttpServletRequest request,HttpSession httpSession,FormDefineVO vo) {
