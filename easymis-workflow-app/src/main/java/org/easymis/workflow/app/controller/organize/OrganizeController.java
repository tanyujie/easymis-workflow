...
@RestController
@Api(value = "组织管理 controller", tags = { "组织管理  操作接口" })
...
public class OrganizeController {
	...
	public RestfulMessage register(HttpSession httpSession, @PathVariable("id") String id) {
		try {
			httpSession.getAttribute("userLogin");
			service.delete(id);
			return new RestfulMessage().success("删除组织成功");
	...
	//获取注册验证码
	@RequestMapping(value = "/register/sendMessage/{uuid}/{mobile}", method = RequestMethod.POST)
	public RestfulMessage registerMobile(HttpSession httpSession, @PathVariable("mobile") String mobile) {
		try {
			httpSession.getAttribute("userLogin");
	...
	//重置密码
	@RequestMapping(value = "/findPassword/verifyAccount/{id}", method = RequestMethod.DELETE)
	public RestfulMessage findPassword(HttpSession httpSession, @PathVariable("id") String id) {
		try {
			httpSession.getAttribute("userLogin");
			service.delete(id);
			return new RestfulMessage().success("删除组织成功");
